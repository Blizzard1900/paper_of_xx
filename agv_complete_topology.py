...
def build_graph():
    ...
    for lane in range(4):
        x = 1 + lane * 0.5
        top = f"Lane_{lane}_top"
        bottom = f"Lane_{lane}_bottom"

        G.add_node(top, pos=(x, 34.0))
        G.add_node(bottom, pos=(x, -31.8))

        edges.append((top, bottom))
        edges.append((bottom, top))
        edges.append((top, "N9"))
        edges.append(("N9", top))
        edges.append((bottom, "N10"))
        edges.append(("N10", bottom))
...
def configure_plot_font():
    """Prefer a Chinese-capable font and gracefully fall back to ASCII."""
    preferred_fonts = [
        "Noto Sans CJK SC",
        "Source Han Sans SC",
        "WenQuanYi Zen Hei",
...
def draw_graph(G, output_path="agv_complete_topology.png"):
    ...
    nx.draw_networkx_edges(G, pos, edgelist=main_edges_list, edge_color="red", width=2.5)

    lane_edges = [(f"Lane_{i}_top", f"Lane_{i}_bottom") for i in range(4)]
    lane_edges += [(f"Lane_{i}_bottom", f"Lane_{i}_top") for i in range(4)]
    nx.draw_networkx_edges(G, pos, edgelist=lane_edges, edge_color="blue", width=2)

    nx.draw_networkx_nodes(G, pos, node_size=20, node_color="black", alpha=0.5)

    key_nodes = ["N1", "N2", "N3", "N5", "N6", "N7", "N8", "N9", "N10"]
    key_labels = {node: node for node in key_nodes if node in pos}

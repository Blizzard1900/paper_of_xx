...
def shortest_distance(instance: Instance, source: str, target: str) -> float:
    return float(nx.shortest_path_length(instance.graph, source, target, weight="distance"))


def shortest_time(instance: Instance, source: str, target: str) -> float:
    return float(nx.shortest_path_length(instance.graph, source, target, weight="base_time"))


def shortest_energy(instance: Instance, source: str, target: str) -> float:
    return float(nx.shortest_path_length(instance.graph, source, target, weight="energy_cost"))
